...
class caps_att(nn.Module):
    ...
    def forward(self, query, feat, feat_mask=None): #query:(64,512) feat:(64,100,512) feat_mask:(64,1,1,100)
        """
        routing algorithm.
        Args:
        ...
        feat = feat @ self.W_f
        b = Variable(torch.zeros(feat.shape[0], feat.shape[1])).cuda()  #b[64,100]
        ...
        for i in range(self.num_iterations):
            ...
            if i == self.num_iterations-1:
                ...
                #norm_factor = torch.norm(query, dim = 1,keepdim=True)* torch.norm(feat,dim=1.keepdim=True)
                #agreement_score =
                delta_b = (delta_b - delta_b.mean(dim=1, keepdim=True)) / (delta_b.std(dim=1, keepdim=True) + 1e-9)
                #b_history = []
                
                '''
                b_history.append(delta_b)
                
                if len(b_history)> self.window_size:
                   b_history.pop(0)
                
                delta_b = sum(b_history)/len(b_history)
                '''

                
                b = b + delta_b
                # outputs = (c.unsqueeze(-1) * feat).sum(dim=1)  # b,dim 64,100
