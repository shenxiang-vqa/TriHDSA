...
from openitm.loader.load_data_itm import DataSet, DataSet_Neg
from openitm.loader.filepath_itm import Path
from openitm.utils.optimizer import WarmupOptimizer
from openitm.utils.sampler import SubsetDistributedSampler
from openitm.utils.itm_loss import BCE_Loss, Margin_Loss
from openitm.models.model_loader import ModelLoader
...
def parse_args():
    ...
    parser.add_argument('--NW', dest='NUM_WORKERS', default=8,
...
class Execution:
    ...
    def get_optim(self, net, search=False, epoch_steps=None):
        ...
        if self.__C.NET_OPTIM == 'sgd':
            net_optim = torch.optim.SGD(net.module.net_parameters() if search else net.parameters(),
                                        self.__C.NET_LR_BASE, momentum=self.__C.NET_MOMENTUM,
    ...
    def train(self, train_loader, eval_loader, neg_caps_loader, neg_imgs_loader):
        ...
        all_frcn_feat_iter_list, all_bbox_feat_iter_list = neg_imgs_loader.dataset.get_all_imgs()
        print('loading finished')
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            if self.__C.RANK == 0:
                logfile = open('./logs/log/log_' + self.__C.VERSION + '_' + str(self.__C.SEED) + '.txt', 'a+')
            ...
            if epoch % self.__C.NEG_NEPOCH == 0 and epoch >= self.__C.NEG_START_EPOCH:
                ...
                with torch.no_grad():
                    ...
                    neg_caps_idx_list = []
                    for step, (frcn_feat_iter_list, bbox_feat_iter_list, cap_ix_iter_list, neg_idx_list) in enumerate(
                            tqdm.tqdm(neg_caps_loader)):
                        frcn_feat_iter_list = frcn_feat_iter_list.view(-1, self.__C.FRCNFEAT_LEN,
                                                                       self.__C.FRCNFEAT_SIZE)
                        ...
                        input = (frcn_feat_iter_list, bbox_feat_iter_list, cap_ix_iter_list)

                        scores_1, scores_2 = net(input)
                       
                        scores_1 = scores_1.view(-1, self.__C.NEG_RANDSIZE)
                        scores_2 = scores_2.view(-1, self.__C.NEG_RANDSIZE)
                        ...
                        scores_ind = neg_idx_list.to(self.__C.DEVICE_IDS[0])[arg_scores_bi, arg_scores].to(
                            scores.device)
                    ...
                    neg_imgs_idx_list_gather = [torch.zeros_like(neg_imgs_idx_list.unsqueeze(1)) for _ in
                                                range(self.__C.WORLD_SIZE)]
                    dist.all_gather(neg_imgs_idx_list_gather, neg_imgs_idx_list.unsqueeze(1))
                    neg_imgs_idx_list_gather = torch.cat(neg_imgs_idx_list_gather, dim=1).view(-1,
                                                                                               self.__C.NEG_HARDSIZE).cpu()  # torch.Size([145000, 20])
            ...
            for step, (train_frcn_feat, train_bbox_feat, train_cap_ix,
                       train_neg_frcn_feat, train_neg_bbox_feat, train_neg_cap_ix) in enumerate(
                tqdm.tqdm(train_loader)):
                train_input_pos = (train_frcn_feat, train_bbox_feat, train_cap_ix)
                train_input_negc = (train_frcn_feat, train_bbox_feat, train_neg_cap_ix)
                train_input_negi = (train_neg_frcn_feat, train_neg_bbox_feat, train_cap_ix)

                # network step
                net_optim.zero_grad()
                scores_pos_1, scores_pos_2 = net(train_input_pos)
                scores_negc_1, scores_negc_2 = net(train_input_negc)
                scores_negi_1, scores_negi_2 = net(train_input_negi)

                loss_1 = loss_fn(scores_pos_1, scores_negc_1, scores_negi_1)
                loss_2 = loss_fn(scores_pos_2, scores_negc_2, scores_negi_2)
                loss_3 = loss_kl(scores_pos_1, scores_negc_1, scores_negi_1)
                loss_4 = loss_kl(scores_pos_2, scores_negc_2, scores_negi_2)
    ...
    def eval(self, eval_loader, net=None, valid=False, redump=False):
        ...
        with torch.no_grad():
            cap_ix_iter_list = eval_loader.dataset.get_all_caps()
            frcn_feat_iter_list, bbox_feat_iter_list = eval_loader.dataset.get_all_imgs()
            ...
            for step_y in tqdm.tqdm(range(row_y)):
                ...
                for step_x in range(col_x):
                    ...
                    eval_input_pos = (frcn_feat_iter, bbox_feat_iter, cap_ix_iter)
                    scores_pos_1, scores_pos_2 = net(eval_input_pos)
                    scores_mat[start_y, start_x: end_x] = scores_pos_1 + scores_pos_2
